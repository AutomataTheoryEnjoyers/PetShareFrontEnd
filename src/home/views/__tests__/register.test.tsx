...
describe("NewAdopter", () => {
  ...
  it("renders the default version of the registration form (default is for Adopter)", () => {
    ...
    expect(
      screen.getByLabelText("Select a role for your account*")
    ).toBeInTheDocument();
  ...
  it("changes the radio button to Shelter", () => {
    ...
    const radioInput = screen.getByText("Shelter");
    expect(screen.getByText("Shelter")).not.toBeChecked();
    fireEvent.click(radioInput);
    expect(screen.getByText("Shelter")).toBeChecked();
  });

  it("calls usePostAdopter when the form is submitted for an Adopter", async () => {
    const mockPostNewAdopter = jest.fn();
